...
/******************************************************************************/
// Prepare old data
use "$hlodta\HLO_Metadata_29Aug2018.dta", clear
	keep cntabb year test subject grade
	replace subject="math" if subject=="" & cntabb=="LKA"
	
//isid cntabb year test subject grade
tempfile hlo
save `hlo'

// Merge old and new data
use "$hlodta\HLO_MEAN_DSEX_SE_v02.dta", clear
	merge 1:1 cntabb year test subject grade using "`hlo'"

// Identify new tests
tab year if _m!=3
//_m==2 is empty PASEC 2014 grade 2
gen hlo_new=1 if _m==1
drop if _m==2
drop _m
...
*******************************************************************************/
	...
	drop if cntabb=="SLV" & test=="TIMSS" & year==2007
	drop if cntabb=="SLV" & test=="LLECE" & year==2006 & subject!="reading" 
	drop if cntabb=="SLV" & test=="LLECE" & year==2006 & grade=="6"
	replace test = "EGRANR" if cntabb=="SLV" & test=="EGRA" & year==2018
	//replace test = "LLECE-reading" if cntabb=="SLV" & test=="LLECE" & year==2006
	...
	*===============================================================================
	drop if grade<"7" & test=="TIMSS" & year==2007 & inlist(cntabb,"DZA","BIH","LBN", "MAR", "UKR") //Only affects Algeria, Morocco, and Ukarine
	drop if grade<"7" & test=="TIMSS" & year==2011 & inlist(cntabb,"MAR","SAU", "UKR","MKD") //Only affects Morocco and Saudi Arabia, others don't have test for primary
	drop if grade<"7" & test=="PIRLS" & year==2011 & inlist(cntabb,"MAR","SAU", "UKR","MKD") //Only affects Morocco and Saudi Arabia, others don't have test for primary
...
collapse hlo* os*, by(wbcode year test subject primary)
...
collapse (mean) hlo_exrt hlo_exrt_se hlo_mf hlo_mf_lower hlo_mf_upper ///
	...
	hlo_mf_unused_source hlo_m_unused_source hlo_f_unused_source hlo_new, by(wbcode year)  

// Order data
order wbcode year test hlo_new ///		
...
// Merge test scores with master data
	merge 1:1 wbcode year using "$otherdta\masterdata.dta"
		drop _m
...
// For numeric variables
	foreach var in hlo_new hlo_m hlo_f hlo_mf ///
...
// Save dataset
sa "$hlodta_out\hlo_20Aug2020.dta", replace	
sa "$hlodta_out\hlo.dta", replace	
