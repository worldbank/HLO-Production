...
use "$path/051_input/Metadata_HLO_sd.dta", replace
...
save "$path/temp/master.dta", replace

*-----------------------------------------
* Create indvidual datasets for each test
*-----------------------------------------

use "$path/temp/master.dta", replace
    ...
    rename se `var'se
    rename sd `var'sd
    save "$path/temp/`var'_`sub'_`level'.dta", replace
    }
    }
    }
...
*-----------------------------------------
    ...
    cap use "$path/temp/`anchor'_`sub'_`lev'.dta", replace
    cap nearmrg cntabb using "$path/temp/`ref'_`sub'_`lev'.dta", nearvar(year) limit(4) roundup type(m:m) genmatch(linkyear)
    ...
    save "$path/temp/doubloons_`anchor'_`ref'_`sub'_`lev'", replace
    ...
    save "$path/temp/link_`anchor'_`ref'_`sub'_`lev'", replace
    }
    }
    }
    }
...
use "$path/temp/link_PISA_TIMSS_math_sec.dta", replace
    save "$path/temp/master2.dta", replace

    foreach anchor in PISA SACMEQ LLECE PASEC EGRA MLA PASEC_2014 {
        foreach ref in TIMSS PIRLS SACMEQ PASEC {
        foreach sub in math reading science {
        foreach lev in sec pri {
    use "$path/temp/master2.dta", replace
            cap append using "$path/temp/link_`anchor'_`ref'_`sub'_`lev'"
            save "$path/temp/master2.dta", replace
        }
        }
        }
...
save "$path/temp/xchange_new.dta", replace 
...
use "$path/temp/master.dta", replace
	...
	merge m:m test level subject using "$path/temp/xchange_new.dta"
...
save "$path/temp/HLO_nature.dta", replace

*--------- Robustness Tests File

use "$path/temp/master.dta", replace
    replace year = 2006 if cntabb == "TGO" & test == "PASEC" & year == 2014  // keep Togo for within PASEC conversion
	merge m:m test level subject test using "$path/temp/xchange_new.dta"
...
save "$path/temp/robustness.dta", replace
