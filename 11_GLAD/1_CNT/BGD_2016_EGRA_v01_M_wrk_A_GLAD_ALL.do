...
local dofile_info = "last modified by Domenica Avila 18.1.2022"  /* change date*/
*
* Steps:
...
set sortseed 10051990
    ...
    noi disp as res "{phang}Step 1 completed (`output_file'){p_end}"
	    ...
	    *According to Shahana at UNICEF Bangladesh, all the data where consent is not equal to 1 should be dropped - Only applies to RCT
    ...
    *---------------------------------------------------------------------------
    ...
    /* NOTE: the merge / append of all rawdata saved in temp in above step
	   ...
	   drop if year < 2015 & "`file'" == "GPS"
	   replace data = "`file'" if missing(data)
	   }
	   *Just one file
    ...
    *<_score_assessment_subject_pv_>
	...
	egen read_comp_score_sum_GPS = rowtotal(comp?) if data == "GPS"
	replace read_comp_score_sum_GPS= read_comp_score_sum_GPS + comp10 if data=="GPS"
	egen read_comp_score_sum_NNPS = rowtotal(comp__? comp__??) if data == "NNPS"
	egen read_comp_score_sum_RCT = rowtotal(mid_comp1_? mid_comp1_??) if data == "RCT"
	gen read_comp_score_sum = read_comp_score_sum_GPS if data == "GPS"
	replace read_comp_score_sum = read_comp_score_sum_NNPS if data == "NNPS"
	replace read_comp_score_sum = read_comp_score_sum_RCT if data == "RCT"
	...
	*<_idgrade_> - From report
	...
	replace idgrade  = mid_class if idgrade==.
