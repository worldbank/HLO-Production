...
gen assessment = test 
merge m:1 assessment level subject using "$clone/02_exchangerate/023_output/exchange_rates.dta", assert(master match using) keep(master match)
...
cf _all using "$clone\03_HLO\033_output\HLO_MEANS_DSEX_v01.dta", verbose
save "$clone\03_HLO\033_output\HLO_MEANS_DSEX_v01.dta", replace
