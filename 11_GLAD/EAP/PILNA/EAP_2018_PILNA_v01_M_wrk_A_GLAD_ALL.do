...
*=========================================================================*
  ...
  *if (_rc == 601) | (`overwrite_files') {
    ...
    *Appending Literacy files:
	   ...
	   foreach file in P18_Y4_Lit_WB P18_Y6_Lit_WB {
    ...
    *---------------------------------------------------------------------------
	 ...
	 */  
       ...
       use "`temp_dir'/P18_Y4_Lit_WB.dta", clear
    ...
    *</_idlearner_>
	...
	order grade gender country schoolauthority schoollocality nmcount idgrade idcntry_raw year idlearner weight
	
	ds grade gender country schoolauthority schoollocality nmcount idgrade idcntry_raw year idlearner weight, not
	local list = r(varlist)
	foreach var of local list {
		tab `var'
		clonevar `var'_cleaned = `var'
		replace `var'_cleaned = "0" if inlist(`var',"9","i","n","r")
		destring `var'_cleaned, replace
		tab `var'_cleaned
	}
	ds *_cleaned
	local cleaned_items = r(varlist)
	egen score_pilna_lit_1 = rowtotal(`cleaned_items')
	egen score_pilna_writ_1 = rowtotal(qoi_cleaned sao_cleaned gas_cleaned punct_cleaned spell_cleaned vocab_cleaned)
	gen score_pilna_read_1 = score_pilna_lit_1 - score_pilna_writ_1
	
	*Converting to percentage correct - Dividing by maximum possible points = 53 , as obtained from the sheet Item desc.
	
	replace score_pilna_lit_1 = (score_pilna_lit_1/53)*100
	replace score_pilna_writ_1 = (score_pilna_writ_1/26)*100
	replace score_pilna_read_1 = (score_pilna_read_1/27)*100
	...
	*<_fpc_>
	...
	Note from 2018 Regional Report: Five of the largest countries were sampled, while 10 countries conducted a census administration.*/
	*Five largest countries: PNG, Fiji, Solomon Islands, Vanuatu and Samoa
	gen fpc = 1  if inlist(country,"598","242","90","548","882")
	replace fpc = 0 if missing(fpc)
	*</_fpc_>
    ...
    *</_escs_>
    ...
    noi disp as res "{phang}Step 4 completed (`output_file'){p_end}"
	  ...
	  save "`temp_dir'/TEMP_`surveyid'_4.dta", replace
	  
	  *-------------------------------------------------------------------------
	  * Repeating for Grade 6
	  *-------------------------------------------------------------------------
       use "`temp_dir'/P18_Y6_Lit_WB.dta", clear
    ...
    *</_idlearner_>
	...
	order grade gender country schoolauthority schoollocality weight nmcount idgrade idcntry_raw year idlearner
	
	ds grade gender country schoolauthority schoollocality weight nmcount idgrade idcntry_raw year idlearner, not
	local list = r(varlist)
	foreach var of local list {
		tab `var'
		clonevar `var'_cleaned = `var'
	...
	*Converting to percentage correct - Dividing by maximum possible points = 57 , as obtained from the sheet Item desc.
	...
	replace score_pilna_writ_1 = (score_pilna_writ_1/26)*100
	replace score_pilna_read_1 = (score_pilna_read_1/31)*100
    ...
    *</_escs_>
    ...
    noi disp as res "{phang}Step 4 completed (`output_file'){p_end}"
	  ...
	  save "`temp_dir'/TEMP_`surveyid'_6.dta", replace
	  append using "`temp_dir'/TEMP_`surveyid'_4.dta"
