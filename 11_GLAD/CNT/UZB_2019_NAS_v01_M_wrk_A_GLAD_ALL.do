...
local region      = "UZB"   /* LAC, SSA, WLD or CNT such as KHM RWA */
local year        = "2019"  /* 2015 */
local assessment  = "NAS" /* PIRLS, PISA, EGRA, etc */
...
local dofile_info = "last modified by Syedah Aroob Iqbal 20th March, 2020"  /* change date*/
*
* Steps:
...
*=========================================================================*
  ...
  *if (_rc == 601) | (`overwrite_files') {
    ...
    /* NOTE: Some assessments will loop over `prefix'`cnt' (such as PIRLS, TIMSS),
       ...
       // Temporary copies of the 4 rawdatasets needed for each country (new section)	*Only Croele data included: 
         if `from_datalibweb'==1 {
           ...
           use "`input_dir'/nas_2020_06_02", clear
         }
         rename *, lower
         compress
         save "`temp_dir'/nas.dta", replace
    ...
    *</_idlearner_>
    ...
    local valuevars	"score_nas* "

    *<_score_assessment_subject_pv_>
    *foreach pv in 01 02 03 04 05 {
	clonevar score_nas_math = ability_timss
      label var score_nas_math "Plausible value `pv': `assessment' score for math"
    *}
    *</_score_assessment_subject_pv_>
    ...
    *<_urban_> - Urban already available and clean.
    ...
    *</_male_>
    ...
    local samplevars "learner_weight su1 su2 national_level nationally_representative regionally_representative"
