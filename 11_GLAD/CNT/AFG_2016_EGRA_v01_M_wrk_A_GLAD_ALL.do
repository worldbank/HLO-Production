...
local dofile_info = "last modified by Domenica Avila 23.1.2022"  /* change date*/
*
* Steps:
...
*=========================================================================*
 ...
 cap set seed "$master_seed"

  // Set up folders in clone and define locals to be used in this do-file
 cap glad_local_folder_setup , r("`region'") y("`year'") as("`assessment'") ma("`master'") ad("`adaptation'")
...
set sortseed 10051990
    ...
    *foreach pv in 01 02 03 04 05 {
	clonevar score_egra_read_d = readcomp_pct
	 ...
	 gen score_egra_read = score_egra_read_d
	 replace score_egra_read = score_egra_read_p if missing(score_egra_read)
	 replace score_egra_read = 0 if missing(score_egra_read)
    *}
    *</_score_assessment_subject_pv_>
...
*/
    ...
    *merge m:1 idcntry_raw using "`temp_dir'/countrycode_list.dta", keep(match) assert(match using) nogen
    ...
    local valuevars : list valuevars | resultvars
		...
		*<_language_test_> 
	clonevar language_test = language /*language of instruction*/ 
	*<_language_test_>
...
drop if idgrade==.
bysort idgrade language_test : sum (score_egra_read)  /*Detected differences between idgrade and grade */
