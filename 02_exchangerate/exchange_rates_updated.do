...
foreach assessment in PISA  {
	...
	foreach s of local sub {
		...
		foreach l of local lev {
			...
			*Morocco data is not nationally representative and therefore not used for linking
			if "`assessment'" == "EGRA" {
			...
			*Collapsing scores by window for each country: 
			keep cntabb year window test subject level score se n
			keep if test == "`reference_assessment'"
			foreach var of varlist test year score se n {
				ren `var' `var'_`reference_assessment'
			}
			save "$clone/02_exchangerate/temp/`assessment'_`reference_assessment'_formerge.dta", replace
			
			use "$clone/02_exchangerate/temp/`assessment'_`s'_`l'_windows.dta", replace
			keep if test == "`assessment'"
			
			merge m:1 cntabb window using "$clone/02_exchangerate/temp/`assessment'_`reference_assessment'_formerge.dta"	
			save "$clone/02_exchangerate/temp/`assessment'_`reference_assessment'_`s'_`l'_windows.dta", replace
			export excel using "$clone/02_exchangerate/temp/`assessment'_`reference_assessment'_`s'_`l'_windows.xls", replace firstrow(variables)
