...
*Note: the HLO_Metadata_29Aug2018.dta dataset is directly retrieved from the Network folder, which is part of the 2018 HCI workflow

use "${clone}/04_creating_hlos/041_rawdata/HLO_Metadata_29Aug2018.dta", clear
  keep cntabb year test subject grade
	replace subject="math" if subject=="" & cntabb=="LKA"
save "${clone}/04_creating_hlos/043_output/HLO_Metadata_reshaped.dta", replace
 
use "${clone}/03_harmonization/033_output/HLO_MEAN_DSEX_SE_v01.dta", clear
	merge 1:1 cntabb year test subject grade using "${clone}/04_creating_hlos/043_output/HLO_Metadata_reshaped.dta"
  

// Identify new tests
tab year if _merge!=3
//_m==2 is empty PASEC 2014 grade 2
gen hlo_new=1 if _merge==1
drop if _merge==2
drop _merge
...
/* NOTE: Using same code as below but with local path 
...
// Merge test scores with master data
	merge 1:1 wbcode year using "${clone}/04_creating_hlos/041_rawdata/masterdata.dta"
		drop _merge
...
save "${clone}/04_creating_hlos/043_output/hlo_new.dta", replace	

*Note: consider renaming the hlo_new.dta file

/******************************************************************/
/******************* ANNEX ***************************************/
//NOTE: The following codes were commented out in the end in Ritika's final .do file from the HCI 2020 update 
/*******************************************************************/
/*******************************************************************/
